...
\begin{tikzpicture}[
    ...
    \begin{axis}[
        ...
        \node [above,color=black,right] at (mt) {$\scriptstyle \dmu$};
    ...
    \begin{axis}[
        ...
        \node [color=black,right] at (tm) {$\scriptstyle \rwdtrustpredict\sim(\mu_t,\sigma_t)$};

        \draw [dotted,opacity=0.5] (2.5,0.0) -- (2.5,2);
        \path[<-,draw] (2.5,0.25) to[out=-70,in=90] (8,-1.5) node[](cm) {}; 
        \node [color=black,left] at (cm) {$\scriptstyle \rwdcandsim\sim(\mu_c,\sigma_c)$};
    ...
    \begin{axis}[
        ...
        \addlegendimage{gray,thick}\addlegendentry{Surrogate \surrogate}
